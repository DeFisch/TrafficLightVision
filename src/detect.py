    ...
    for i in range(3):
        ...
        if circles[i] is not None:
            ...
            for circle in circles[i][0, :]:
                ...
                if bgr_var < 0  or (i == 0 and (18 < avg_hue < 160 or avg_sat < 120 or avg_val < 170)) or (i == 1 and (not(18 < avg_hue < 33) or not(200 < avg_val < 255))) or (i == 2 and (not(50 < avg_hue < 95))):
                    del_idx.append(j)
                elif circle[1] > 0.75*height:
                    del_idx.append(j)
                else:
                    print(f'[{i}: {circle[0]}]: hue = {avg_hue}, sat = {avg_sat}, val = {avg_val}, rad = {rad}')
                j+=1
